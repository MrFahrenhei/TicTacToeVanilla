export default class View{
    ...
    render(game, stats){
        ...
        this.#closeAll();
        this.#clearMoves();
        let player1wins = playerWithStats[0].wins;
        let player2wins = playerWithStats[1].wins;
        this.#updateScoreboard(player1wins, player2wins, ties);
    ...
    #qs(selector, parent){
        ...
        if(!el) throw new Error('Could not find a selector');
        return el;
    }
    #qsAll(selector, parent){
        const elList = document.querySelectorAll(selector);

        if(!elList) throw new Error('Could not find a selector');
